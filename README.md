...
* **`GET /api/v1/wallets/{walletId}/statement`** *(self-service, any authenticated caller)* — paginated, newest-first `AccountEntries` for a wallet, filterable by `fromDate`/`toDate`. Ownership-enforced: `404` if the wallet doesn't exist, `403` if the caller doesn't own it (a valid token alone does not authorize reading any wallet's statement). Backed by the existing `IX_AccountEntries_AccountId_CreatedAt` index.
