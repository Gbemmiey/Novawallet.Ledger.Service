...
#### Prompt 16 — "I want to trace wallet credit from webhook to funds application"

Investigation showed the webhook half was already traced by the built-in ASP.NET Core/EF Core/Npgsql instrumentation, but the consumer is a polling `BackgroundService` reading a Postgres outbox, so the trace broke at the table: each settlement started an unrelated trace. The existing `TraceContextInjector`/`Extractor` helpers are RabbitMQ-shaped and unused in this flow, and the web host registered zero custom `ActivitySource`s, so any new source would also have been silently dropped until registered. Fix (user approved the one-column migration): `DepositOutbox.TraceParent` (nullable `varchar(64)`) stores the W3C `traceparent` written inside the existing accept transaction; a new `NovaWalletTracing` source (`NovaWallet.Deposits`, now registered) gives `deposit.accept` -> `deposit.settle` (parented on the stored context, so one trace) with `lock_outbox` / `credit_wallet` / `post_journal` children, retry events, and Error status on failures and business rejections. One non-obvious detail: the consumer must read `TraceParent` in the batch query alongside the id, because an `Activity`'s parent can't be changed once started, so it can't wait until the row is locked inside the transaction. Also avoided `Activity.AddException` (.NET 9 only) by recording the OTel `exception` event by hand. Not compiled or run; the migration is the user's to author.

#### Prompt 17 — "TraceParent in DepositOutbox is null. Can you create JMeter test plans …"

Root-cause hypothesis for the null: `DepositService` built the outbox row inside the `HybridCache.GetOrCreateAsync` factory, and I believe the factory does not reliably flow `ExecutionContext`, so `Activity.Current` was null there. Fix: capture the `deposit.accept` activity and `traceparent` in `SubmitDepositRequest` (where they are ambient) and pass them through the factory's state tuple; the `deposit.replay` tag now uses the captured activity too. This is an inference, not something I reproduced - rows written before a rebuild stay null regardless. Five JMeter plans (creation, statement, credit, transfer, concurrent multi-wallet) live in `loadtest/`, generated by a small script. My earlier plan had the credit and transfer routes wrong (`/api/v1/credit`); reading `Program.cs` showed both sit under `/api/v1/wallets`. Neither the code change nor the plans were compiled or run.
